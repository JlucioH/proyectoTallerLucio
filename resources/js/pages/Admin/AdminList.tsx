...
export default function AdminList({ empresas }: Props) {
    ...
    const filteredData = useMemo(() => {
        return empresas.map(empresa => ({
            ...empresa,
            clientes: empresa.clientes.filter(c => 
                c.razonSocialCliente.toLowerCase().includes(searchTerm.toLowerCase()) ||
                (c.nitFacturaCliente || '').includes(searchTerm) ||
                empresa.nombreEmpresa.toLowerCase().includes(searchTerm.toLowerCase())
            )
        })).filter(empresa => empresa.clientes.length > 0);
    }, [empresas, searchTerm]);

    return (
        <AppSidebarLayout breadcrumbs={[{ title: 'Admin / Consolidado Clientes', href: '/admin/clientes-consolidado' }]}>
            <Head title="Consolidado de Clientes" />
            ...
            <div className="flex flex-1 flex-col gap-4 p-4 py-6 md:p-8 text-slate-900 dark:text-slate-100">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b pb-6 dark:border-slate-800">
                    <div>
                        ...
                        <p className="text-sm text-slate-500 font-medium">Vista general de clientes por empresa</p>
                    </div>
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="text-left">
                                            ...
                                            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-tight">NIT: {empresa.nitEmpresa} • {empresa.clientes.length} Clientes</p>
                                        </div>
                                    </div>
                                </button>
                                ...
                                {expandedEmpresas.includes(empresa.idEmpresa) && (
                                    <div className="p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 border-t dark:border-slate-800">
                                        {empresa.clientes.map(cliente => (
                                            <div key={cliente.idCliente} className="p-3 rounded-lg border dark:border-slate-800 bg-white dark:bg-slate-900 flex flex-col gap-2">
                                                <div className="flex justify-between items-start">
                                                    <span className="text-[10px] font-black text-indigo-600 dark:text-indigo-400 uppercase">#{cliente.codigoCliente || 'S/C'}</span>
                                                    <span className={`h-2 w-2 rounded-full ${cliente.estadoCliente ? 'bg-emerald-500' : 'bg-red-500'}`}></span>
                                                </div>
                                                <h3 className="font-bold text-sm leading-tight line-clamp-1">{cliente.razonSocialCliente}</h3>
                                                <div className="text-[11px] text-slate-500 space-y-1">
                                                    <div className="flex items-center gap-2"><Phone className="h-3 w-3" /> {cliente.celularCliente || '--'}</div>
                                                    <div className="flex items-center gap-2"><Mail className="h-3 w-3" /> {cliente.correoCliente || '--'}</div>
                                                    {cliente.nitFacturaCliente && <div className="mt-1 font-bold text-indigo-500">NIT: {cliente.nitFacturaCliente}</div>}
                                                </div>
                                            </div>
                                        ))}
                                    </div>
